...
def is_regression(candidate_verify: dict, champion: dict | None) -> bool:
    ...
    # pass_rate tolerance scales with battery size: on a 7-8-prompt battery a
    # SINGLE flipped item moves pass_rate by 12-14 points, so a fixed 3-point
    # tolerance fired regressions on pure judge noise. avg_score stays tight.
    n = candidate_verify.get("total") or champion.get("total") or 0
    pr_tol = max(REGRESSION_TOLERANCE, (100.0 / n) + 1.0) if n else REGRESSION_TOLERANCE
    return (cand["pass_rate"] < champ - pr_tol
...
def promote(artifact_dir: Path, verify: dict) -> dict | None:
    ...
    rec = {
        ...
        # Battery size — needed to scale the pass_rate regression tolerance.
