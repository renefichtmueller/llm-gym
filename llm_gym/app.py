...
from .queue import TrainingQueue, _verdict
...
def _catalog_sync_loop() -> None:
    """Background freshness loop: periodically pull the live shop catalog so the
    adapters stay current on new products (snapshot refresh for grounding/retrieval
    + controlled new-product ingestion into the sales pools). Daemon thread — rides
    the reboot-proof gym daemon, so no cron/launchd needed. Gated by settings."""
    import threading
    import time as _t
    from . import catalog_sync

    def _run() -> None:
        _t.sleep(max(0, settings.catalog_sync.startup_delay_sec))
        while True:
            try:
                cs = settings.catalog_sync
                if cs.enabled:
                    catalog_sync.sync(settings, ingest=cs.ingest, log=lambda m: None)
            except Exception:
                pass
            _t.sleep(max(3600, settings.catalog_sync.interval_hours * 3600))

    threading.Thread(target=_run, daemon=True, name="catalog-sync").start()


if getattr(settings, "catalog_sync", None) and settings.catalog_sync.enabled:
    _catalog_sync_loop()
...
def api_deploy(name: str) -> dict:
    ...
    return deploy.build_and_register(spec, _store().artifact_dir(name), _ollama(), log,
                                     gpu_lease_path=settings.gpu_lease_path)
...
@app.post("/api/adapters/{name}/security")
def api_security(name: str) -> dict:
    """Red-team the trained adapter against the held-out injection battery:
    resistance on attack prompts + non-over-refusal on benign-contrast prompts.
    Defense in depth alongside the gateway. Returns the robustness score + breakdown."""
    spec = _store().load(name)
    if not spec:
        raise HTTPException(404, "No such adapter")
    from .trainer import select_backend
    ...
    backend = select_backend(settings.backend).name
    return security.injection_redteam(spec, _store().artifact_dir(name), backend, settings)


@app.post("/api/adapters/{name}/distill")
def api_distill(name: str, dry_run: bool = False, limit: int = 0) -> dict:
    """Promote judge-'perfect' collected pairs into the gold split (wires the
    previously-dead score_pair_quality gate). dry_run counts without writing.
    GPU-bound (the judge runs on Ollama) — run at idle, not during training."""
    spec = _store().load(name)
    if not spec:
        raise HTTPException(404, "No such adapter")
    from . import distill as _distill
    return _distill.distill(spec, settings, limit=limit, dry_run=dry_run)


@app.post("/api/adapters/{name}/eval/bootstrap")
def api_eval_bootstrap(name: str, n: int = 40) -> dict:
    """Freeze a held-out eval.jsonl from the adapter's gold (when none exists) so
    verify compares champion vs candidate on a fixed, never-trained bench instead of
    the leaking auto-split. No-op if a bench already exists."""
    spec = _store().load(name)
    if not spec:
        raise HTTPException(404, "No such adapter")
    return Pool(settings.pool_path, spec.pool or name).eval_bootstrap(n=n)
...
def api_live() -> dict:
    ...
    # Whoever actually holds the shared GPU lease right now, independent of the
    # `jobs`-table 'running' row below — a pipeline sub-stage (check/verify/
    # head-to-head/red-team/deploy) holds this same lease without ever
    # appearing in `jobs`, so a queued job can wait behind it for a long time
    # with zero visibility into what it's actually waiting on. Computed once
    # here so both `waiting_for_gpu` below and the dashboard's "waiting" card
    # can show who's holding it and (for red-team) live progress + an ETA.
    gpu_holder = None
    try:
        if settings.gpu_lease_path:
            import sys as _sys
            p = os.path.expanduser(settings.gpu_lease_path)
            if p not in _sys.path:
                _sys.path.insert(0, p)
            import gpu_queue as _gq
            held = (_gq.status() or {}).get("running")
            if held:
                kind, _, name = held.get("owner", "").partition(":")
                started_h = held.get("start") or 0
                gpu_holder = {
                    "owner": held.get("owner", ""), "kind": kind,
                    "adapter": name or held.get("owner", ""),
                    "info": held.get("info", ""), "started": started_h,
                    "elapsed_s": int(_time.time() - started_h) if started_h else 0,
                }
                if kind == "llm-gym-pipeline-redteam":
                    from . import security as _security
                    prog = _security.redteam_progress()
                    if prog and prog.get("adapter") == name:
                        gpu_holder["progress"] = prog
    except Exception:  # noqa: BLE001
        pass
    train = None
    if running:
        ...
        # A job is 'running' at the DB level the instant _run() starts — well
        # before it actually gets the shared GPU lease, and it can then sit
        # queued behind another adapter's (possibly long) train/verify/red-team
        # stage for a long time. Found live: the dashboard showed "running ⏱
        # 117:36" for a job that had never produced a single training iteration
        # — indistinguishable from a genuine hang. Surface the true state: if
        # there's no iteration yet, check whether THIS job actually holds the
        # real GPU lease right now, or is just waiting its turn.
        waiting_for_gpu = (progress.get("iter") is None and gpu_holder is not None
                          and gpu_holder["owner"] != f"llm-gym:{running['adapter']}")
        train = {
    ...
    for r in (q.get("recent") or [])[:12]:
        ...
        if v:
            ...
            # Honest verdict: when an acceptance verify exists, grade by pass-rate
            # (comparable across data shapes) instead of the val-loss verdict, which
            # mislabels free-prose lanes 'weak' for a loss that is normal for prose.
            if v.get("pass_rate") is not None:
                row["verdict"] = _verdict(None, None, None, None, True,
