...
function renderVerify(v) {
  $("plVerifyCard").style.display = "block";
  $("plVerdict").textContent = `${v.pass_rate}% pass · avg ${v.avg_score}`;
...
document.querySelectorAll("#plGateTier button[data-tier]").forEach((b) =>
  b.addEventListener("click", async () => {
    await post(`/api/adapters/${PL.adapter}/pipeline/gate`, { tier: b.dataset.tier });
    startPipelinePolling();
  }));
document.querySelectorAll("#plGatePromote button[data-promote]").forEach((b) =>
  b.addEventListener("click", async () => {
    await post(`/api/adapters/${PL.adapter}/pipeline/gate`, { promote: b.dataset.promote === "yes" });
    startPipelinePolling();
  }));
...
async function loadLive() {
  ...
  if (t) {
    ...
    fill($("liveTrain"),
      ...
      h("div", { class: "live-meta" }, badge("running", "ok"), " ",
  ...
  (d.pipelines || []).forEach((pl) => {
    ...
    if (pl.status === "running") { st = badge("active", "ok"); sub = pl.last || ""; }
      ...
      sub = v && v.pass_rate != null
        ? `${tr(BAND_LABEL[pl.band] || pl.band || "")} · ${v.pass_rate}% · avg ${v.avg}`
          + (pl.regression ? "  ⚠ " + tr("worse than the live version") : "")
    ...
    acts.push(h("div", { class: "live-row" + (pl.status === "running" ? " live-active" : "") },
      h("div", null, head), sub ? h("div", { class: "muted mono small" }, sub) : null, gate));
  });
