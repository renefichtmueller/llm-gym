...
def build_and_register(spec: AdapterSpec, artifact_dir: Path, client: OllamaClient,
                       log: Callable[[str], None], *, min_free_gb: float = 20.0,
                       gpu_lease_path: str = "") -> dict:
    ...
    `fuse` re-materialises the full base model (dequantized) and is genuinely
    GPU/memory-heavy — it MUST hold the same shared lease queue.py's training uses,
    or it can co-load with an active training job on the single-GPU host (the exact
    class of bug hardened against elsewhere this week: two GPU consumers running
    unserialized). Held for the whole fuse->convert->quantize span, not just fuse.
    """
    from .queue import _gpu_lease  # local import: avoids a queue<->deploy cycle
    with _gpu_lease(gpu_lease_path, f"llm-gym-deploy:{spec.name}", "adapter deploy (fuse/gguf/quantize)"):
        return _build_and_register(spec, artifact_dir, client, log, min_free_gb=min_free_gb)
