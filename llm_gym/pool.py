...
class Pool:
    ...
    def append(self, examples: list[dict], split: str = "train") -> dict:
        """Append valid examples to a split (train|valid|gold|eval|collected|raw).
        Skips dupes already present in train+gold."""
        target = {
    ...
    def build_merged(self, min_tier: str | None = None) -> dict:
        """Deduplicated union of gold + train (+ collected at/above min_tier), with
        cross-split leakage removed: examples present in valid OR the frozen eval
        set are dropped, so the held-out eval can never leak into training."""
        from . import scoring
        held_out = self._read(self.valid_file) + self._read(self.eval_file)
        valid_hashes = {_hash_messages(e["messages"]) for e in held_out
                        if _valid_example(e)}
    ...
    def eval_examples(self) -> list[dict]:
        """The FROZEN, held-out eval set (curated). It is NEVER used in training
        (build_merged's leakage guard drops it), so champion/candidate numbers come
        from the same fixed questions every run instead of a churning reshuffle."""
        return [e for e in self._read(self.eval_file) if _valid_example(e)]

    def eval_bootstrap(self, n: int = 40, seed: int = 9) -> dict:
        """Seed a frozen eval.jsonl from existing GOLD (deterministic sample) when
        none exists — a stable bench beats the leaking auto-split. The chosen rows
        are then auto-excluded from training by build_merged. No-op if one exists."""
        existing = self.eval_examples()
        if existing:
            return {"ok": False, "reason": "eval set already exists", "eval": len(existing)}
        import random
        gold = [e for e in self._read(self.gold_file) if _valid_example(e)]
        if len(gold) < 24:
            return {"ok": False, "reason": f"too little gold ({len(gold)}) to spare a held-out bench"}
        take = min(n, max(8, len(gold) // 4))   # at most a quarter — keep the rest for training
        order = list(range(len(gold)))
        random.Random(seed).shuffle(order)
        added = self._append(self.eval_file, [gold[i] for i in order[:take]])
        return {"ok": True, "eval": added, "from": "gold", "gold_remaining": len(gold) - added}
