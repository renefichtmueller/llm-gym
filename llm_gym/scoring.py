...
# A *grounded* abstention — "that SKU isn't in our catalog, let me verify" or
# "which switch model exactly?" — is the CORRECT behaviour for catalog-grounded
# lanes (sales/support that must never invent SKUs/prices), NOT junk. When a
# refusal phrase co-occurs with one of these constructive/verifiable signals we
# exempt it from the non-answer penalty, so we stop training the model AWAY from
# honest abstention. A bare dodge (no such signal) is still penalised.
_GROUNDED_OK = (
    "catalog", "katalog", "verify", "let me check", "let me confirm", "double-check",
    "look it up", "look up", "prüfe", "prüfen", "nachschauen", "nachsehen",
    "which model", "which switch", "welches modell", "welcher switch", "welche reichweite",
    "serial", "seriennummer", "could you provide", "could you share", "können sie",
    "bitte um", "part number", "artikelnummer", "in our shop", "im shop", "im katalog",
)
...
def score_example(example: dict, objective_tokens: set[str], *, unique: bool = True) -> dict:
    ...
    low = answer.lower()
    if any(p in low for p in _NON_ANSWER) and not any(g in low for g in _GROUNDED_OK):
        penalty += 45
    # Catalog grounding: a SKU-shaped token that is NOT in the real product
    # catalog is a hallucinated part number — hard-penalise it so invented-SKU pairs
    # can never reach a usable tier. No-ops when the catalog snapshot is absent or
    # the answer cites no SKUs (lazy, lru-cached read — keeps this path effectively pure).
    try:
        from . import catalog as _catalog
        if not _catalog.check_grounding(answer).get("ok", True):
            penalty += 45
    except Exception:
        pass
