...
## Secrets & Vaultwarden (optional)

The gym has two outbound secrets — the **Git push token** (`LLMGYM_GIT_TOKEN`) and
the optional **public-judge API key** (`LLMGYM_JUDGE_API_KEY`). Both are read from
the environment and never written to disk; the Git token is injected only into the
push URL and is stripped from `.git/config`.

If you run a **Vaultwarden** or **Bitwarden** vault you can keep these in your vault
instead of plaintext env files:

1. Point the `bw` CLI at *your own* vault: `bw config server https://your-vault` then `bw login`.
2. At runtime, unlock once: `export BW_SESSION=$(bw unlock --raw)`.
3. Name the vault item in Settings: `git.token_item` (push token) and/or `judge.key_item` (judge key).

The gym fetches the item's password field at use time and never logs or stores it.
Nothing about any specific vault is baked into the code — you point it at your own.
